...
def solve_shift_scheduling(params: dict):
    ...
    fixed_assignments = params['fixed_assignments']
    ...
    # (shift, hard_min, soft_min, min_penalty,
    # soft_max, hard_max, max_penalty)
    if params['shift_constraints'] is not None:
      shift_constraints = params['shift_constraints']
    else:
      shift_constraints = [
    ...
    # (shift, hard_min, soft_min, min_penalty,
    # soft_max, hard_max, max_penalty)
    if params['weekly_sum_constraints'] is not None:
      weekly_sum_constraints = params['weekly_sum_constraints']
    else:
    ...
    if params['penalized_transitions'] is not None:
      penalized_transitions = params['penalized_transitions']
    else:
    ...
    if params['weekly_cover_demands'] is not None:
      weekly_cover_demands = params['weekly_cover_demands']
    else:
    ...
    if params['excess_cover_penalties'] is not None:
      excess_cover_penalties = params['excess_cover_penalties']
    else:
    ...
    # inputs
    inputs = {}
    inputs['num_employees'] = num_employees
    inputs['num_weeks'] = num_weeks 
    inputs['shifts'] = shifts 
    inputs['fixed_assignments'] = fixed_assignments 
    inputs['requests'] = requests
    inputs['shift_constraints'] = shift_constraints
    ...
    if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
        ...
        for i, var in enumerate(obj_bool_vars):
            if solver.boolean_value(var):
                ...
                if penalty > 0:
                    penalties = f"{penalties}\n  {var.name} violated, penalty={penalty}"
                else:
                    penalties = f"{penalties}\n  {var.name} fulfilled, gain={-penalty}"#

        for i, var in enumerate(obj_int_vars):
            if solver.value(var) > 0:
                penalties = f"{penalties}\n  {var.name} violated by {solver.value(var)}, linear"
                penalties = f"{penalties}  penalty={obj_int_coeffs[i]}"
...
if __name__ == "__main__":
    parms = {}
    parms['num_employees'] = 8
    parms['num_weeks'] = 3
    parms['shifts'] = ["O", "M", "A", "N"]
    parms['fixed_assignments'] = []
    parms['requests'] = [
        (6, 1, 1, 21),
        (6, 2, 1, 21),
        (6, 3, 1, 21),
    ...
    parms['shift_constraints'] = None
    parms['weekly_sum_constraints'] = None
    parms['penalized_transitions'] = None
    parms['weekly_cover_demands'] = [
        (2, 3, 1),  # Monday
        (2, 3, 1),  # Tuesday
        (2, 2, 2),  # Wednesday
        (2, 3, 1),  # Thursday
